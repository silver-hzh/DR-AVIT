...
parser.add_argument('--config', type=str, default='configs/demo_edges2handbags_folder.yaml',
...
image_names_A = ImageFolder(opts.input_folder_A, transform=None, return_paths=True)
data_loader_A = get_data_loader_folder(opts.input_folder_A, 1, False, new_size=config['new_size_a'], crop=False)

image_names_B = ImageFolder(opts.input_folder_B, transform=None, return_paths=True)
data_loader_B = get_data_loader_folder(opts.input_folder_B, 1, False, new_size=config['new_size_a'], crop=False)
...
if opts.trainer == 'MUNIT':
    ...
    for i, (images, names) in enumerate(zip(data_loader_A, image_names_A)):
        if opts.compute_CIS:
            cur_preds = []
        print(names[1])
        images = Variable(images.cuda(), volatile=True)
